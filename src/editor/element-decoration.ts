 ...
 * half the blur. Only images and figures carry these decorations today; a
 * riquadro has no shadow yet.
 */
export function elementDecorationExtent(
  attrs: Record<string, unknown> | undefined,
  typeName: string | undefined,
): DecorationExtent {
  if (typeName !== "image" && typeName !== "figure") return ZERO;
  const s = normalizeImageStyle(attrs ?? {});
  const frame = s.borderWidth > 0 && s.borderStyle !== "none" ? s.borderWidth : 0;
  let x = frame;
  let y = frame;
  if (s.shadowEnabled) {
    const { dx, dy } = bearingToOffset((s.shadowAngle + 180) % 360, s.shadowDistance);
    const spread = frame; // matches computeImageBoxShadow
