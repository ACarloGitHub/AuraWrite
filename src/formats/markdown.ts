  ...
  switch (node.type.name) {
    ...
    case "bullet_list":
      return node.content?.map((item: any) => {
        const itemContent = item.content?.map(nodeToMarkdown).join("") || "";
...
export function fromMarkdown(markdown: string): any {
  ...
  while (i < lines.length) {
    ...
    if (line.match(/^\d+\.\s+/)) {
      ...
      content.push({ type: "ordered_list", attrs: { order: 1 }, content: items });
      continue;
    }
...
function parseInlineMarkdown(text: string): any[] {
  ...
  while (remaining.length > 0) {
    ...
    if (strongMatch) {
      result.push({ type: "text", text: strongMatch[1], marks: [{ type: "strong" }] });
